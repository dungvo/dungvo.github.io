...
const words=story=>[story.title_vi,story.subtitle_vi,...(story.sections||[]).flatMap(s=>(s.blocks||[]).map(b=>b.text_vi)),story.takeaway?.text_vi,...(story.reflection?.prompts_vi||[]),story.reflection?.closing_vi].filter(Boolean).join(' ').trim().split(/\s+/).length;
...
function resolveHero(story){
  ...
  return descriptor?bases[els.channel.value]+descriptor.path:null;
}
...
function render(){
  ...
  els.scroll.scrollTop=0;updateProgress();syncURL();
}

function introduction(story,heroURL){
  const fragment=document.createDocumentFragment();
  if(heroURL){const hero=document.createElement('div');hero.className='hero';const image=document.createElement('img');image.src=heroURL;image.alt=story.hero_image?.alt_text_vi||'';hero.append(image);fragment.append(hero)}
