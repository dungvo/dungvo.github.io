...
function storyBody(story){const body=document.createElement('div');body.className='story-body';(story.sections||[]).forEach(section=>{const node=document.createElement('section');node.className='story-section';node.id=section.id;const blocks=[...(section.blocks||[])];while(blocks[0]?.type==='part_heading')node.append(blockNode(blocks.shift()));if(section.title_vi){const heading=document.createElement('h2');heading.textContent=section.title_vi;node.append(heading)}blocks.forEach(block=>node.append(blockNode(block)));body.append(node)});return body}
